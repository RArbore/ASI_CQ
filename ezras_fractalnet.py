...
class CIFARFractalNet(nn.Module): #Model class
    ...
    """
    def __init__(self,  #Input: (B, 3, 256, 256), Output: (B, paletteSize * 3)
                 channels,
                 num_columns,
                 dropout_probs,
                 loc_drop_prob,
        ...
        self.num_classes = paletteSize*3
        self.glob_drop_ratio = glob_drop_ratio
        self.num_columns = num_columns

    #Declaring convolutional layers
        self.conv = ParametricSequential()
        for i, out_channels in enumerate(channels):
            dropout_prob = dropout_probs[i]
            self.conv.add_module("unit{}".format(i + 1), FractalUnit(
                in_channels=in_channels,
                out_channels=out_channels,
                num_columns=num_columns,
                loc_drop_prob=loc_drop_prob,
